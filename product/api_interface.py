...
# 已废弃 SQLAlchemy ORM，改用 pymysql
# from sqlalchemy.orm import Session, selectinload
# from sqlalchemy.sql import or_, text
from pydantic import BaseModel, Field, field_validator

# 已废弃：from database_setup import get_db
# 请使用统一的 pymysql 连接：from config import get_conn
from config import get_conn
from .models import ProductStatus  # 仅保留枚举类，不再使用 ORM 模型
from config import BASE_PIC_DIR, CATEGORY_CHOICES
...
def register_routes(app):
    """注册商品管理路由到主应用"""
    from .product_ext import router as product_ext_router
...
def reorder_product_dict(p) -> Dict[str, Any]:
    """从 ORM 对象构建商品字典（已废弃，保留用于兼容，仅用于向后兼容）"""
    # 此函数已废弃，不再使用
    # 如果传入的是字典，直接使用 build_product_dict
    if isinstance(p, dict):
        return build_product_dict(p)
    # 如果是 ORM 对象（向后兼容）
    base = {col: getattr(p, col) for col in PRODUCT_COLUMNS}
    base["skus"] = [{"id": s.id, "sku_code": s.sku_code, "price": float(s.price),
                     "stock": s.stock} for s in p.skus] if hasattr(p, 'skus') else []
    base["attributes"] = [{"name": attr.name, "value": attr.value} for attr in p.attributes] if hasattr(p, 'attributes') else []
    base["freight"] = 0.00
    return base

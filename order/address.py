...
class AddressManager:
    # ------------- 新增地址 -------------
    @staticmethod
    def add(user_id: int, label: str, name: str, mobile: str,
            ...
            with conn.cursor() as cur:
                ...
                cur.execute("""
                    ...
                    (user_id, label, consignee_name, consignee_phone,
                     province, city, district, detail, lng, lat, is_default)
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                """, (user_id, label, name, mobile, province, city, district,
                      detail, lng, lat, int(is_default)))
                conn.commit()
    ...
    def list_by_user(user_id: int) -> List[Dict[str, Any]]:
        with get_conn() as conn:
            with conn.cursor() as cur:
                cur.execute("""
                    SELECT id, label, consignee_name AS name, consignee_phone AS phone,
                           province, city, district, detail, lng, lat, is_default, created_at
                    FROM user_addresses
                    WHERE user_id=%s
    ...
    def set_default(addr_id: int, user_id: int) -> bool:
        with get_conn() as conn:
            with conn.cursor() as cur:
                cur.execute("UPDATE user_addresses SET is_default=0 WHERE user_id=%s", (user_id,))
                cur.execute("UPDATE user_addresses SET is_default=1 WHERE id=%s AND user_id=%s",
                            (addr_id, user_id))
                conn.commit()
    ...
    def delete(addr_id: int, user_id: int) -> bool:
        with get_conn() as conn:
            with conn.cursor() as cur:
                cur.execute("DELETE FROM user_addresses WHERE id=%s AND user_id=%s",
                            (addr_id, user_id))
                conn.commit()
                return cur.rowcount > 0
...
def add_address(body: AddressAdd, user_id: int):
    addr_id = AddressManager.add(
        user_id, body.label, body.name, body.mobile,
        body.province, body.city, body.district, body.detail,
        body.lng, body.lat, body.is_default)
    return {"id": addr_id}
